...
```jsonc
{
  ...
  // [Optional] Read memory range data from a separate block device / volume
  // instead of source_url. Other snapshot data (config, state JSON) is still
  // read from source_url.
  "memory_vol_url": "file:///dev/vdb",
...
|------|---------|-------------|
| `--memory-vol <url>` | (none) | Write memory range data to a separate volume (e.g. `file:///dev/vdb`). Other snapshot data (config, state JSON) is still written to the snapshot path. |

Example:
```bash
cube-runtime snapshot \
  --path /data/snapshots/my-snap \
  --memory-vol file:///dev/vdb \
  --vm-id <sandbox_id>
```
