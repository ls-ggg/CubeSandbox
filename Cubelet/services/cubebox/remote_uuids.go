...
// before Pause / Commit / AppSnapshot returns. XFS is a no-op. Export
// failure is logged and returns empty — the customer-facing RPC still
// succeeds; Master records remote_status=failed without uuids (same-node
// resume does not need them; cross-node needs ready + uuids).
